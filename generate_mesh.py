...
def resize_dem(source_dem, x_res, y_res):
	return gdal.Warp(resized_filename, source_dem, xRes=x_res, yRes=y_res)


def clip_dem(source_dem, downscale):
	...
	if downscale:
		...
		new_x = res * gt[1]
		new_y = res * gt[5]
		clipped_dem = resize_dem(clipped_dem, new_x, new_y)
...
if __name__ == "__main__":
	...
	clip_dem(merged_filename, True)
	dem_to_mesh("Africa_resized.tif")
